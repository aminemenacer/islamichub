      ...
      <div class="col-md-6 order-2 order-md-1 pb-3">
        <div class="mt-3 mt-md-0">
          ...
          <div class="mb-3">
            ...
            <input type="number" id="donationAmount" v-model="amount" class="form-control stripe-input" placeholder="Enter amount" min="1" step="1" required />
          </div>

          <!-- Donate Button -->
          <button class="btn btn-primary w-100" @click="donate" :disabled="!stripe || amount <= 0">
            Donate
          </button>
...
export default {
  ...
  mounted() {
    loadStripe(process.env.MIX_STRIPE_PUBLISHABLE_KEY).then((stripeInstance) => {
  ...
  methods: {
    async donate() {
      ...
      try {
        ...
        if (!sessionId) {
          this.errorMessage = "Failed to initiate checkout session. Please try again.";
          return;
        }
